...
import { hideCursor, showCursor, clearLines, animationsEnabled } from "./tty.ts";
...
export async function showMenu(opts: MenuOpts): Promise<string | null> {
  ...
  function render(linesPrinted: number) {
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      if (isActive) {
        console.log(`${arrow} ${rgb(palette.sky, num)}${rgb(palette.sky, label)}${dim(desc)}`);
      } else {
        console.log(`${arrow} ${dim(num)}${dim(label)}${dim(desc)}`);
      }
      printed++;
    }
    // blank line + footer
    ...
    function onData(key: string) {
      ...
      if (/^[1-9]$/.test(key)) {
        ...
        if (n < items.length) {
          active = n;
          printed = render(printed);
          // Auto-confirm on number-press is a Mole-ism; we mirror it.
          cleanup();
          resolve(items[active].value);
          return;
        }
