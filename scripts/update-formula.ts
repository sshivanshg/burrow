#!/usr/bin/env bun
/**
 * Update Formula/burrowed.rb with new version + sha256s. Used by the
 ...
 *
 *   bun scripts/update-formula.ts dist/SHA256SUMS Formula/burrowed.rb 0.3.0
 */
...
const arm = sha.match(/^([a-f0-9]{64})\s+burrowed-darwin-arm64$/m)?.[1];
const x64 = sha.match(/^([a-f0-9]{64})\s+burrowed-darwin-x64$/m)?.[1];
if (!arm || !x64) {
